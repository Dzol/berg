...
%% @doc Predicate: is this the zero value of a heap? It'll fail for
%% something that does not have the structure of a heap. True if it is
%% a zero heap. False if the heap has cardinality greater than or
%% equal to one.
-spec zero(heap())                          -> boolean().
zero({infinity, []})                        -> true;
...
merge({X, _} = H, {Y, V}) when Y =< X -> {Y, [H|V]}.
